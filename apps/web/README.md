...
Contexto específico do app Next.js que vive em `apps/web`.

- **Scripts úteis** (executar na raiz):
  - `pnpm dev` – sobe `next dev apps/web`
  - `pnpm build` / `pnpm start` – build e servidor de produção
  - `pnpm lint` / `pnpm check` / `pnpm test` – lint, type-check e testes
- **Env**: crie `.env.local` na raiz do monorepo; Next lê para este app.
- **Aliases**: `@/` aponta para `apps/web/src`.
