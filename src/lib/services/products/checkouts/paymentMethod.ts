...
export const paymentMethodRequests = {
  ...
  getCheckoutPaymentMethod: async (
    productId: string,
    checkoutId: string,
    token?: string
  ): Promise<unknown> => {
    ...
    return (
      (await tryGet(`/product/${productId}/checkouts/${checkoutId}/paymentMethods`)) ??
      (await tryGet(`/product/${productId}/checkouts/${checkoutId}/paymentMethod`))
    );
  },
  ...
  updateCheckoutPaymentMethod: async (
    ...
    if (!paymentMethodId) {
      response = await request(`/product/${productId}/checkouts/${checkoutId}/paymentMethods`, {
        method: "POST",
        baseUrl: PRODUCTS_BASE,
        headers: {
          "Content-Type": "application/json",
