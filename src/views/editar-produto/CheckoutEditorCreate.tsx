...
export function CheckoutEditorCreate({
  ...
  const [notificationRules, setNotificationRules] = useState<
    { id: string; label: string; enabled: boolean; min: string }[]
  >([
    { id: "buying", label: "XX pessoas estão comprando o produto.", enabled: false, min: "" },
    { id: "just-bought", label: "XX pessoas compraram o produto agora mesmo.", enabled: false, min: "" },
    { id: "last-30", label: "XX pessoas compraram o produto nos últimos 30 minutos.", enabled: false, min: "" },
    { id: "last-hour", label: "XX pessoas compraram o produto na última hora.", enabled: false, min: "" },
  ]);
  ...
  const DEFAULT_AFTER_SALE_TITLE = "Agradecemos pela sua compra!";
  const DEFAULT_AFTER_SALE_MESSAGE = "Você receberá um e-mail confirmando o seu pedido.";
  const [afterSaleTitle, setAfterSaleTitle] = useState("");
  const [afterSaleMessage, setAfterSaleMessage] = useState("");
  const [currentAfterSaleTitle, setCurrentAfterSaleTitle] = useState("");
  const [currentAfterSaleMessage, setCurrentAfterSaleMessage] = useState("");
  ...
  useEffect(() => {
    const fetchCheckout = async () => {
      ...
      try {
        ...
        const rawCheckout = (data ?? {}) as unknown as Record<string, unknown>;
        const checkoutPayload =
          (rawCheckout.checkout as Record<string, unknown> | undefined) ??
          (rawCheckout.data as Record<string, unknown> | undefined) ??
          rawCheckout;
        const coerceRecord = (value: unknown) =>
          value && typeof value === "object" ? (value as Record<string, unknown>) : null;
        const coerceFirstRecord = (value: unknown) =>
          Array.isArray(value) ? coerceRecord(value[0]) : coerceRecord(value);
        const paymentFromCheckout =
          coerceFirstRecord(checkoutPayload.productCheckoutPayment) ??
          coerceFirstRecord(checkoutPayload.productCheckoutPayments) ??
          coerceFirstRecord(checkoutPayload.product_checkout_payment) ??
          coerceFirstRecord(checkoutPayload.product_checkout_payments) ??
          coerceFirstRecord(checkoutPayload.paymentMethod) ??
          coerceFirstRecord(checkoutPayload.payment_method) ??
          null;
        let paymentSource: Record<string, unknown> | null = paymentFromCheckout;
        let resolvedPaymentMethodId =
          (paymentFromCheckout?.id as string | undefined) ??
          (checkoutPayload.productCheckoutPaymentId as string | undefined) ??
          (checkoutPayload.paymentMethodId as string | undefined) ??
          (checkoutPayload.payment_method_id as string | undefined);

        try {
          const paymentMethodsList = await productApi.getCheckoutPaymentMethods(
            productId,
            checkoutId,
            token
          );
          console.log("[checkout] PaymentMethods GET:", paymentMethodsList);
          const directMethod = coerceRecord(paymentMethodsList);
          const list = Array.isArray(paymentMethodsList)
            ? paymentMethodsList
            : (paymentMethodsList as { data?: unknown; paymentMethods?: unknown } | null)?.paymentMethods ??
              (paymentMethodsList as { data?: unknown } | null)?.data ??
              [];
          if (directMethod && !Array.isArray(paymentMethodsList)) {
            paymentSource = directMethod;
            resolvedPaymentMethodId = (directMethod.id as string | undefined) ?? resolvedPaymentMethodId;
          } else if (Array.isArray(list) && list.length) {
            const firstMethod = coerceRecord(list[0]);
            if (firstMethod) {
              paymentSource = firstMethod;
              resolvedPaymentMethodId = (firstMethod.id as string | undefined) ?? resolvedPaymentMethodId;
            }
          }
        } catch (error) {
          console.error("Erro ao buscar payment methods do checkout:", error);
        }
        ...
        try {
          ...
          const messagesList = Array.isArray(messages)
            ...
            : (messages as { messages?: unknown; data?: unknown } | null)?.messages ??
              ...
              [];
          let resolvedMessage: Record<string, unknown> | null = null;
          let resolvedMessageId: string | undefined;
          if (Array.isArray(messagesList) && messagesList.length) {
            const first = coerceRecord(messagesList[0]);
            resolvedMessageId = (first?.id as string | undefined) ?? undefined;
            resolvedMessage = first;
            if (resolvedMessageId) {
              try {
                ...
                resolvedMessage =
                  coerceRecord(detail) ??
                  coerceRecord((detail as { data?: unknown } | null)?.data) ??
                  resolvedMessage;
              } catch (error) {
          ...
          const resolvedTitle =
            (resolvedMessage?.title as string | undefined) ??
            (resolvedMessage?.after_sale_title as string | undefined) ??
            (checkoutPayload.after_sale_title as string | undefined) ??
            "";
          const resolvedMessageText =
            (resolvedMessage?.message as string | undefined) ??
            (resolvedMessage?.after_sale_message as string | undefined) ??
            (checkoutPayload.after_sale_message as string | undefined) ??
            "";
          setCurrentAfterSaleTitle(resolvedTitle);
          setCurrentAfterSaleMessage(resolvedMessageText);
          setAfterSaleTitle("");
          setAfterSaleMessage("");
        } catch (error) {
        ...
        const loadedName =
          (checkoutPayload.name as string | undefined) ??
          (checkoutPayload.checkoutName as string | undefined) ??
          "Checkout";
        setCurrentCheckoutName(loadedName);
        setCheckoutName("");
        setTheme((checkoutPayload.theme as "dark" | "light") || "dark");
        setAccentColor(
          (checkoutPayload.defaultColor as string | undefined) ??
            (checkoutPayload.default_color as string | undefined) ??
            "#5f17ff"
        );
        const normalizedLogo = normalizeImageUrl((checkoutPayload.logo as string | null | undefined) ?? "");
        const normalizedBanner = normalizeImageUrl((checkoutPayload.banner as string | null | undefined) ?? "");
        setShowLogo(Boolean(normalizedLogo));
        setLogoPosition((checkoutPayload.position_logo as "left" | "center" | "right") || "left");
        ...
        setBannerDataUrl(null);
        setRequiredAddress((checkoutPayload.required_address as boolean | undefined) ?? true);
        setRequiredPhone((checkoutPayload.required_phone as boolean | undefined) ?? true);
        setRequiredBirthdate((checkoutPayload.required_birthdate as boolean | undefined) ?? true);
        setRequiredDocument((checkoutPayload.required_document as boolean | undefined) ?? true);
        setRequiredEmailConfirmation(Boolean(checkoutPayload.required_email_confirmation));
        setCountdownEnabled(
          Boolean(checkoutPayload.countdown_active || checkoutPayload.countdown)
        );
        ...
        const acceptCompany =
          ...
          (paymentSourceFallback?.acceptDocumentCompany as boolean | undefined) ??
          false;
        const acceptIndividual =
          (paymentSourceFallback?.accept_document_individual as boolean | undefined) ??
          (paymentSourceFallback?.acceptDocumentIndividual as boolean | undefined) ??
          false;
        const acceptPix =
          (paymentSourceFallback?.accept_pix as boolean | undefined) ??
          (paymentSourceFallback?.acceptPix as boolean | undefined) ??
          false;
        const acceptCard =
          (paymentSourceFallback?.accept_credit_card as boolean | undefined) ??
          (paymentSourceFallback?.acceptCreditCard as boolean | undefined) ??
          false;
        const acceptTicket =
          (paymentSourceFallback?.accept_ticket as boolean | undefined) ??
          (paymentSourceFallback?.acceptTicket as boolean | undefined) ??
          false;
        const acceptCoupon =
          (paymentSourceFallback?.accept_coupon as boolean | undefined) ??
          (paymentSourceFallback?.acceptCoupon as boolean | undefined) ??
          false;
        const shownSeller =
          (paymentSourceFallback?.shown_seller_detail as boolean | undefined) ??
          (paymentSourceFallback?.shownSellerDetail as boolean | undefined) ??
          false;
        const requireAddressPreference =
          (paymentSourceFallback?.require_address as boolean | undefined) ??
          (paymentSourceFallback?.requireAddress as boolean | undefined);
        ...
        setAcceptedDocuments(acceptDocs.length ? acceptDocs : []);
        setPaymentMethods(acceptPayments.length ? acceptPayments : []);
        setCouponEnabled(Boolean(acceptCoupon));
        setShowSellerDetail(Boolean(shownSeller));
        if (typeof requireAddressPreference === "boolean") {
          setPreferenceRequireAddress(requireAddressPreference);
        }

        const detailDiscount =
          (paymentSourceFallback?.detail_discount as Record<string, unknown> | undefined) ??
          (paymentSourceFallback?.detailDiscount as Record<string, unknown> | undefined);
        const detailPayment =
          (paymentSourceFallback?.detail_payment_method as Record<string, unknown> | undefined) ??
          (paymentSourceFallback?.detailPaymentMethod as Record<string, unknown> | undefined);
        ...
        setSocialProofMinClient(
          (checkoutPayload.social_proofs_min_client as string | number | undefined) != null
            ? String(checkoutPayload.social_proofs_min_client)
            : ""
        );
        const payloadTestimonials = Array.isArray(checkoutPayload.testimonials)
          ? (checkoutPayload.testimonials as Array<{
              id?: string;
              name?: string;
              text?: string;
              rating?: number;
        ...
        setReviewsEnabled(
          Boolean(checkoutPayload.testimonials_enabled) || resolvedTestimonials.length > 0
        );
  ...
  const handleSave = async () => {
    ...
    if (!acceptedDocuments.length) {
      notify.error("Selecione ao menos um documento (CPF/CNPJ).");
      return;
    }
    if (!paymentMethods.length) {
      notify.error("Selecione ao menos um método de pagamento.");
      return;
    }
    const shouldSendPaymentMethods = paymentMethods.length > 0;
    ...
    const finalAfterSaleTitle =
      afterSaleTitle.trim() || currentAfterSaleTitle.trim() || DEFAULT_AFTER_SALE_TITLE;
    const finalAfterSaleMessage =
      afterSaleMessage.trim() || currentAfterSaleMessage.trim() || DEFAULT_AFTER_SALE_MESSAGE;
    payload.after_sale_title = finalAfterSaleTitle;
    payload.after_sale_message = finalAfterSaleMessage;
    ...
    try {
      ...
      if (targetCheckoutId && productId) {
        try {
          const messagePayload = {
            title: finalAfterSaleTitle,
            message: finalAfterSaleMessage,
          };
          console.log("[checkout] Payload mensagem pós-compra:", messagePayload);
          if (messageId) {
            await productApi.updateCheckoutMessage(
              productId,
              targetCheckoutId,
              messageId,
              messagePayload,
          ...
          notify.error("Erro ao salvar mensagem pós-compra.");
        }
      }
      if (targetCheckoutId && productId && token && shouldSendPaymentMethods) {
        try {
          ...
          if (createdId) {
            // id reservado caso seja necessário em fluxos futuros
          }
        } catch (error) {
    ...
    <div className="w-full px-4 py-8">
      <div className="mx-auto flex w-full max-w-6xl 2xl:max-w-7xl flex-col items-stretch gap-6">
        ...
        <div className="flex flex-col gap-3 overflow-y-auto pb-6">
          ...
          <div className="space-y-3 w-full max-w-[470px]">
            <PaymentsSection
            ...
            <SectionCard title="Pós-compra" iconSrc="/images/editar-produtos/poscompra.svg">
              <div className="space-y-3">
                <label className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <input
                    className={fieldClass}
                    placeholder={currentAfterSaleTitle || DEFAULT_AFTER_SALE_TITLE}
                ...
                <label className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <textarea
                    ...
                    placeholder={currentAfterSaleMessage || DEFAULT_AFTER_SALE_MESSAGE}
