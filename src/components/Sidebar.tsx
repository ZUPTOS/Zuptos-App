      ...
      <nav className="flex flex-col gap-2 p-4">
        {navItems.map((item) => {
          ...
          const iconWrapperClasses = `flex ${
            isExpanded ? "h-10 w-14" : "h-10 w-16"
          ...
          return (
            <Link
              key={item.id}
              href={item.href}
              className={`group relative flex w-full items-center gap-2 rounded-[7px] border border-card py-2 transition-all duration-300 ${
                isActive ? "text-foreground" : ""
              } ${isExpanded ? "px-4" : "justify-center px-2 border-transparent"}`}
              title={item.label}
            >
              {isActive && (
                <span className="absolute -left-2 h-8 w-[3px] bg-primary" />
              )}
